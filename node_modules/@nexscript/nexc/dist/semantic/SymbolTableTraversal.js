...
export default class SymbolTableTraversal extends AstTraversal {
    ...
    visitContract(node) {
        ...
        const unusedParameterNames = node.parameters
            .filter((parameter) => parameter.unused).map((parameter) => parameter.name);
        const unusedSymbols = node.symbolTable.unusedSymbols()
            .filter((symbol) => !unusedParameterNames.includes(symbol.name));
