...
Input.DEFAULT_TYPE = DEFAULT_TYPE;
...
Input.prototype._fromObject = function(params) {
  ...
  if (params.amount) {
    ...
    this.amountBN = BN.fromNumber(params.amount);
  }
  return this;
};
...
Input.fromBufferReader = function(br) {
  var input = new Input();
  input.type = br.readVarintNum();
  input.prevTxId = br.readReverse(32);
  input._scriptBuffer = br.readVarLengthBuffer();
  input.sequenceNumber = br.readUInt32LE();
  input.amountBN = br.readUInt64LEBN();
  input.amount = input.amountBN.toNumber();;
  return input;
};
...
Input.prototype.toBufferWriterIdem = function(writer) {
  if (!writer) {
    writer = new BufferWriter();
  }
  writer.writeUInt8(this.type);
