...
var PublicKeyTemplateInput = Input.PublicKeyTemplate;
var PublicKeyHashInput = Input.PublicKeyHash;
var PublicKeyInput = Input.PublicKey;
var MultiSigScriptHashInput = Input.MultiSigScriptHash;
var MultiSigInput = Input.MultiSig;
var EscrowInput = Input.Escrow;
var ScriptTemplateInput = Input.ScriptTemplate;
 ...
 * @constructor
 */
function Transaction(serialized = undefined) {
...
Transaction.DUST_AMOUNT = (148 + 34) * 3;
...
Transaction.FEE_PER_KB = 1000 * 3;

// Safe upper bound for change address script size in bytes : type + value + scriptlen + script
Transaction.CHANGE_OUTPUT_MAX_SIZE = 1 + 8 + 1 + 23; //p2pkt
...
var hashProperty = {
  ...
  enumerable: true,
  get: function() {
    this._hash = new BufferReader(this._getIdHash()).readReverse().toString('hex');
    return this._hash;
  }
};
...
var idemProperty = {
  ...
  enumerable: true,
  get: function() {
    this._idem = new BufferReader(this._getIdem()).readReverse().toString('hex');
    return this._idem;
  }
};
Object.defineProperty(Transaction.prototype, 'idem', idemProperty);
...
Transaction.prototype.checkedSerialize = function(opts) {
  ...
  if (serializationError) {
    ...
    //   'https://nexcore.otoplo.com/transaction/#serialization-checks';
...
Transaction.prototype.toBufferIdem = function() {
  var writer = new BufferWriter();
  return this.toBufferWriterIdem(writer).toBuffer();
};

Transaction.prototype.toBufferWriterIdem = function(writer) {
  writer.writeUInt8(this.version);
  writer.writeVarintNum(this.inputs.length);
  _.each(this.inputs, function(input) {
    input.toBufferWriterIdem(writer);
  });
  writer.writeVarintNum(this.outputs.length);
  _.each(this.outputs, function(output) {
    output.toBufferWriter(writer);
...
Transaction.prototype.toBufferSatisfier = function() {
  var writer = new BufferWriter();
  return this.toBufferWriterSatisfier(writer).toBuffer();
};

Transaction.prototype.toBufferWriterSatisfier = function(writer) {
  writer.writeInt32LE(this.inputs.length);
  _.each(this.inputs, function(input) {
    writer.write(input._scriptBuffer);
    writer.writeUInt8(Opcode.map.OP_INVALIDOPCODE);
  });
  return writer;
};
...
Transaction.prototype.fromBufferReader = function(reader) {
  ...
  this.version = reader.readVarintNum();
...
Transaction.prototype.fromObject = function fromObject(arg) {
  ...
  _.each(transaction.inputs, function(input) {
    ...
    var txin;
    if (script.isPublicKeyTemplateOut() || script.isGroupedPublicKeyTemplateOut()) {
      txin = new Input.PublicKeyTemplate(input);
    } else if (script.isScriptTemplateOut() || script.isGroupedScriptTemplateOut()) {
      txin = new Input.ScriptTemplate(
        input, input.templateScript, input.constraintScript, input.visibleArgs, input.publicKey
      );
...
Transaction.prototype.from = function(utxo, pubkeys, threshold, opts) {
  ...
  } else if (utxo.templateScript && utxo.constraintScript && utxo.publicKey) {
    this._fromScriptTemplateUtxo(utxo, utxo.templateScript, utxo.constraintScript, utxo.visibleArgs, utxo.publicKey);
  } else {
    this._fromPubKeyUtxo(utxo);
  }
  return this;
};
...
Transaction.prototype._selectInputType = function(utxo, pubkeys, threshold) {
  ...
  } else if (utxo.script.isPublicKeyTemplateOut() || utxo.script.isGroupedPublicKeyTemplateOut()) {
    clazz = PublicKeyTemplateInput;
  } else if (utxo.script.isScriptTemplateOut() || utxo.script.isGroupedScriptTemplateOut()) {
...
Transaction.prototype._fromPubKeyUtxo = function(utxo) {
  var clazz;
  utxo = new UnspentOutput(utxo);
...
Transaction.prototype._fromScriptTemplateUtxo = function(utxo, templateScript, constraintScript, visibleArgs, pubKey) {
  var clazz;
  utxo = new UnspentOutput(utxo);
  if (utxo.script.isScriptTemplateOut() || utxo.script.isGroupedScriptTemplateOut()) { // general case - we will add more if needed
    clazz = ScriptTemplateInput;
  } else {
    clazz = Input;
  }
  this.addInput(new clazz({
    ...
    script: Script.empty(),
    amount: utxo.satoshis,
    groupId: utxo.groupId,
    groupAmount: utxo.groupAmount
  }, templateScript, constraintScript, visibleArgs, pubKey));
};
 ...
 * @return {Transaction} this, for chaining
 */
Transaction.prototype.to = function(address, amount, outType) {
  if (_.isArray(address)) {
    var self = this;
    _.each(address, function(to) {
      self.to(to.address, to.satoshis, to.type);
    });
    return this;
  }
 ...
 * @return {Transaction} this, for chaining
 */
Transaction.prototype.toGrouped = function(address, groupId, amount) {
  if (_.isArray(address)) {
    var self = this;
    _.each(address, function(to) {
      self.toGrouped(to.address, to.groupId, to.amount);
    });
    return this;
  }
  ...
  this.addOutput(new Output({
    type: 1,
    script: Script.buildGroupedScriptTemplateOut(address, groupId, amount),
 ...
 * @return {Transaction} this, for chaining
 */
Transaction.prototype.addGroupData = function(script) {
  $.checkArgumentType(script, Script, 'script');
  $.checkState(script.isDataOut(), 'script is not OP_RETURN');
  this.addOutput(new Output({
    type: 0,
    script: script,
    satoshis: 0
...
Transaction.prototype._updateOutput = function(index, sats) {
  this.outputs[index].satoshis = sats;
  this._outputAmount = undefined;
  this._updateChangeOutput();
};
...
Transaction.prototype._estimateFee = function () {
  ...
  var feeWithChange = Math.ceil(getFee(estimatedSize) + (this._changeScript ? getFee(Transaction.CHANGE_OUTPUT_MAX_SIZE) : 0));
  if (!this._changeScript || available <= feeWithChange || available - feeWithChange < Transaction.DUST_AMOUNT) {
    return available;
  }
...
Transaction.prototype._estimateSize = function() {
  var result = 4 + 1; // locktime + version
  ...
  result += 1;
  _.each(this.outputs, function(output) {
    result += output.script.toBuffer().length + 1 + 8 + 1;
  });
 ...
 * @return {Transaction} this, for chaining
 */
Transaction.prototype.sign = function(privateKey, sigtype = Signature.SIGHASH_NEXA_ALL) {
  let signingMethod = "schnorr"
...
Transaction.prototype.isCoinbase = function() {
  return (_.isUndefined(this.inputs) || this.inputs.length === 0);
};
