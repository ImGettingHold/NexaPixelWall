...
BN.prototype.toBigInt = function() {
...
BN.prototype.toBuffer = function(opts) {
  ...
  if (opts && opts.size) {
    ...
    if (natlen === opts.size) {
      ...
      buf = BN.trim(buf, natlen, opts.size);
    } else if (natlen < opts.size) {
...
BN.prototype.toBigNumSMBigEndian = function() {
  var buf;
  if (this.cmp(BN.Zero) === -1) {
    buf = this.neg().toBuffer();
    buf = Buffer.concat([Buffer.from([0x80]), buf]);
...
BN.prototype.toSM = function(opts) {
  ...
  var bignum = opts ? opts.bignum : false;
  var buf = bignum ? this.toBigNumSMBigEndian() : this.toSMBigEndian();
...
BN.prototype.toScriptBigNumBuffer = function() {
  return this.toSM({
    endian: 'little',
    bignum: true
  });
...
BN.trim = function(buf, natlen, size) {
  return buf.slice(natlen - buf.length, (natlen - buf.length == 0 ? size : buf.length));
};
