 ...
 * @returns {BufferWriter}
 */
BufferWriter.prototype.writeVarLengthBuf = function(buf) {
  assert(bufferUtil.isBuffer(buf));
 ...
 * @returns {BufferWriter}
 */
BufferWriter.prototype.writeCoreVarintNum = function(n) {
  var tmp = [];
  var len = 0;
 ...
 * @returns {Buffer}
 */
BufferWriter.groupAmountBufBN = function(bn) {
  var bw = new BufferWriter();
  var n = bn.toBigInt();
  if (n < 0n) {
    bn = new BN(BigInt.asUintN(64, n).toString());
    bw.writeUInt64LEBN(bn)
  } else if (n < 0x10000n) {
